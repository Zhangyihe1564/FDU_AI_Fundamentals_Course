\documentclass[UTF8,fontset=windows]{ctexart}  % Article class with built-in Chinese support
\usepackage{geometry}
...
\usepackage{caption}
% Packages for pictures (using graphicx)
\usepackage{graphicx}  % For including images
\graphicspath{{images/}}  % Optional: specify path to image directory
\usepackage{float}  % Provides the H float specifier

% For professional table rules (\toprule, \midrule, \bottomrule)
\usepackage{booktabs}

% Packages for code blocks (using listings)
\usepackage{listings}
...
\begin{enumerate}
    ...
    \item \textbf{测试模型}：在每个 epoch 结束后，使用测试数据集评估模型性能，并记录 \texttt{test\_acc} 和 \texttt{train\_acc} 。
    \item \textbf{绘制损失曲线}：在所有 epoch 训练完成后，将数据传入自定义的draw\_loss\_and\_accuracy\_curve函数，使用 Matplotlib 库绘制训练和测试损失曲线。X 轴表示 epoch 数，Y 轴表示损失值和准确率。
    \item 使用不同颜色的线条区分训练和测试损失曲线及正确率，并添加图例、标题和轴标签以提高可读性。
...
\section{实验}
\subsection{Task1：绘制损失函数曲线}
这部分使用给定的Net结构和优化器，训练LeNet 网络，并在每个epoch 结束后记录训练和测试损失值以及准确率。通过调用\texttt{draw\_loss\_and\_accuracy\_curve}函数绘制损失和准确率曲线。该函数代码如下：
\begin{lstlisting}
...
optimizer = optim.SGD(net.parameters(), lr=0.001, momentum=0.9, weight_decay=0.0001)
    \end{lstlisting}
    \item \textbf{Dropout}：在LeNet的\texttt{\_\_init\_\_}方法中添加Dropout层，并在\texttt{forward}方法中应用。代码示例如下：
    \begin{lstlisting}
    class Net(nn.Module):
        def __init__(self):
...
DenseNet的核心思想是通过密集连接将每一层的输出与后续所有层的输入进行拼接，
...
\end{enumerate}
查阅原始论文知，类似层数的DenseNet-40(with bottleneck)在300轮训练后准确率接近94.06\%。
